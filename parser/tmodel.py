...
from parser.modules import CHAR_LSTM, MLP, BertEmbedding, Biaffine, BiLSTM, TrilinearScorer, Triaffine, Qriaffine
...
class Model(nn.Module):
        ...
        def __init__(self, args):
                ...
                if self.args.use_second_order:
                        ...
                        if args.use_sib:
                                ...
                                self.mlp_sib_rh = MLP(n_in=args.n_lstm_hidden*2,
                                                                 n_hidden=args.n_mlp_sec,
                                                                 dropout=args.mlp_dropout,identity=self.binary)
                                self.mlp_sib_rd = MLP(n_in=args.n_lstm_hidden*2,
                                                                 n_hidden=args.n_mlp_sec,
                                                                 dropout=args.mlp_dropout,identity=self.binary)
                                self.mlp_sib_lh = MLP(n_in=args.n_lstm_hidden*2,
                                                                 n_hidden=args.n_mlp_sec,
                                                                 dropout=args.mlp_dropout,identity=self.binary)
                                self.mlp_sib_ld = MLP(n_in=args.n_lstm_hidden*2,
                                                                 n_hidden=args.n_mlp_sec,
                                                                 dropout=args.mlp_dropout,identity=self.binary)
                                self.mlp_sib_sh = MLP(n_in=args.n_lstm_hidden*2,
                                                                 n_hidden=args.n_mlp_sec,
                                                                 dropout=args.mlp_dropout,identity=self.binary)
                                self.mlp_sib_sd = MLP(n_in=args.n_lstm_hidden*2,
                                                                 n_hidden=args.n_mlp_sec,
                                                                 dropout=args.mlp_dropout,identity=self.binary)
                                self.trilinear_sib_nc = Qriaffine(args.n_mlp_sec,args.n_mlp_sec,args.n_mlp_sec,init_std=args.init_std, rank = args.n_mlp_sec, factorize = True)
                        if args.use_cop:
        ...
        def forward(self, words, feats):
            if self.training:
                ...
                if self.args.use_second_order:
                        ...
                        self.arc_sib = (arc_sib[2], arc_sib[3], arc_sib[4], arc_sib[5], arc_sib[6], arc_sib[7])
        ...
        def encode_second_order(self, memory_bank):
                ...
                if self.use_sib:
                        ...
                        edge_node_sib_lh = self.mlp_sib_lh(memory_bank)
                        edge_node_sib_lm = self.mlp_sib_ld(memory_bank)
                        edge_node_sib_rh = self.mlp_sib_rh(memory_bank)
                        edge_node_sib_rm = self.mlp_sib_rd(memory_bank)
                        edge_node_sib_sh = self.mlp_sib_sh(memory_bank)
                        edge_node_sib_sm = self.mlp_sib_sd(memory_bank)
                        arc_sib=(edge_node_sib_h, edge_node_sib_m, edge_node_sib_lh, edge_node_sib_lm, edge_node_sib_rh, edge_node_sib_rm, edge_node_sib_sh, edge_node_sib_sm)
                else:
                        arc_sib=None
        ...
        def NetEnergy(self, tree, bmask=None, grad=True, verse=False):
                ...
                if self.use_sib:
                    ...
                    edge_node_sib_h_l, edge_node_sib_m_l, edge_node_sib_h_r, edge_node_sib_m_r, edge_node_sib_h_s, edge_node_sib_m_s = self.arc_sib
                    #edge_node_sib_h_l, edge_node_sib_m_l = self.arc_sib
                    ...
                    edge_node_sib_h_s = edge_node_sib_h_s[bmask]
                    edge_node_sib_m_s = edge_node_sib_m_s[bmask]
                    if not grad:
                        ...
                        edge_node_sib_h_s = edge_node_sib_h_s.detach()
                        edge_node_sib_m_s = edge_node_sib_m_s.detach()
                    E_sib = ((q_value.unsqueeze(-1))*(q_value.unsqueeze(-2))*me_sib).sum(dim=(-1,-2,-3))
                    ...
                    edge_node_sib_h_s = torch.matmul(q_value.transpose(-1,-2),edge_node_sib_h_s)
                    layer_sib = self.trilinear_sib_nc(edge_node_sib_m_l, edge_node_sib_m_r, edge_node_sib_m_s, edge_node_sib_h_l, edge_node_sib_h_r, edge_node_sib_h_s) 
                    nidx = torch.arange(tree.size(-1)+1)
                    one_mask=torch.ones(layer_sib.shape[-2:]).cuda()
                    tril_mask=torch.tril(one_mask,-1)
                    
                    if tree.dim()==3:
                        layer_sib[:,nidx,nidx,:] = 0
                        layer_sib[:,:,nidx,nidx] = 0
                        layer_sib[:,nidx,:,nidx] = 0
                        layer_sib = layer_sib-layer_sib*tril_mask.unsqueeze(0).unsqueeze(0)
                        layer_sib = layer_sib-layer_sib*tril_mask.unsqueeze(0).unsqueeze(-1)
                        layer_sib = layer_sib-layer_sib*tril_mask.unsqueeze(0).unsqueeze(-2)
                        nlayer_sib = torch.zeros(layer_sib.size()).cuda()
                        tidx = idx[:-1]
                        if tidx.size(0)!=0: nlayer_sib[:,tidx, tidx+1,tidx+2] = layer_sib[:,tidx, tidx+1,tidx+2]
                        tidx = idx[:-2]
                        if tidx.size(0)!=0:
                            nlayer_sib[:,tidx, tidx+2,tidx+3] = layer_sib[:,tidx, tidx+2,tidx+3]
                            nlayer_sib[:,tidx, tidx+1,tidx+3] = layer_sib[:,tidx, tidx+1,tidx+3]
                        tidx = idx[:-3]
                        if tidx.size(0)!=0: nlayer_sib[:,tidx, tidx+2,tidx+4] = layer_sib[:,tidx, tidx+2,tidx+4]
                        layer_sib = nlayer_sib
                    else:
                        layer_sib[:,:,nidx,nidx,:] = 0
                        layer_sib[:,:,:,nidx,nidx] = 0
                        layer_sib[:,:,nidx,:,nidx] = 0
                        layer_sib = layer_sib-layer_sib*tril_mask.unsqueeze(0).unsqueeze(0).unsqueeze(0)
                        layer_sib = layer_sib-layer_sib*tril_mask.unsqueeze(0).unsqueeze(0).unsqueeze(-1)
                        layer_sib = layer_sib-layer_sib*tril_mask.unsqueeze(0).unsqueeze(0).unsqueeze(-2)
                        nlayer_sib = torch.zeros(layer_sib.size()).cuda()
                        tidx = idx[:-1]
                        if tidx.size(0)!=0: nlayer_sib[:,:,tidx, tidx+1,tidx+2] = layer_sib[:,:,tidx, tidx+1,tidx+2]
                        tidx = idx[:-2]
                ...
                #E_gp = 0
                if not verse: 
                    E = (q_value*unary_potential).sum(dim=(-1,-2)) + E_sib + E_gp + layer_sib.sum(dim=(-1,-2,-3)) #+ layer_gp.sum(dim=(-1,-2,-3)) #+ E_sib_lin.sum(-1) + E_gp_lin.sum(-1)
                    return E
                else: return (q_value*unary_potential).sum(dim=(-1,-2)), E_sib+E_gp, layer_sib.sum(dim=(-1,-2,-3))#+layer_gp.sum(dim=(-1,-2,-3)) 
        ...
        def mean_field_variational_infernece(self):
            ...
            if self.training: 
                ...
                #print('linear: ' + str(torch.abs(El).sum().item()))
                #print('second: ' + str(torch.abs(Es).sum().item()))
                #print('third: ' + str(torch.abs(Et).sum().item()))
                E = El+Es+Et
                E, = grad(E.sum(), ptree)
