...
from parser.utils.alg import inside
...
class Model(nn.Module):
        ...
        def forward(self, words, feats, arcs=None):
            if self.training:
                self.tarcs = arcs
                ...
                self.rmask[:,0] = 0
                self.lengths = self.rmask.sum(dim=1)
                lens = mask.sum(dim=1)
                ...
                if self.args.use_second_order:
                        ...
                        ptree[~self.mask_unary[:,1:,1:]] = 0
                        self.gold_arcs = ptree

                        s_arc = self.mean_field_variational_infernece(mask)
        ...
        def NetEnergy(self, tree, bmask=None, grad=True, verse=False):
                ...
                if not verse: 
                    E = (q_value*unary_potential).sum(dim=(-1,-2)) + E_sib + E_gp
                    return E
                else: return (q_value*unary_potential).sum(dim=(-1,-2)), E_sib+E_gp, 0 
        ...
        def mean_field_variational_infernece(self,mask):
            ...
            if self.training: 
                ...
                ttree = self.gold_arcs.float().requires_grad_()
                tE = self.NetEnergy(ttree)
                tE, = grad(tE.sum(), ttree, create_graph=True)
            else: 
                ...
                E = El+Es+Et
                E, = grad(E.sum(), ptree)
                if self.mbr: E = E.requires_grad_()

            if self.mbr:
                nE = torch.zeros(E.size(0), E.size(1)+1, E.size(2)+1).cuda()
                E = E - (E*ptree.detach()).sum(-2).unsqueeze(-2)
                nE[:,1:,1:] = E.transpose(-1,-2)
                nE[:,1:,0] = E[:,idx,idx]
                nE.masked_fill_(~mask.unsqueeze(1), float('-inf'))
                s_i, s_c = inside(nE, self.rmask)
                logz = s_c[0].gather(0, (self.lengths).unsqueeze(0)).squeeze(0)
                if self.training:
                    E = self.NetEnergy(self.gold_arcs) - self.NetEnergy(ptree.detach())
                    return -(E - logz).sum()/(self.lengths.sum())
                    #E = E-(logz/self.lengths).unsqueeze(-1).unsqueeze(-1)
                else: E, = grad(logz.sum(), E)
            else:
                ...
                if self.training:
                    tE[~self.mask_unary[:,1:,1:].transpose(-1,-2)] = -float('inf')
                    tE = tE - torch.logsumexp(tE, dim=-2).unsqueeze(-2)
                    tE[~self.mask_unary[:,1:,1:].transpose(-1,-2)] = 0
                    E = (E+tE)/2
            nE = torch.zeros(E.size(0), E.size(1)+1, E.size(2)+1).cuda()
            nE[:,1:,1:] = E.transpose(-1,-2)
            nE[:,1:,0] = E[:,idx,idx]
