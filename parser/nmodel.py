...
class Model(nn.Module):
                ...
                if self.args.use_second_order:
                        ...
                        if args.use_sib:
                                ...
                                self.mlp_sib_cd = MLP(n_in=args.n_lstm_hidden*2,
                                                                 n_hidden=args.n_mlp_sec,
                                                                 dropout=args.mlp_dropout,identity=self.binary)
                                self.trilinear_sib_l = TrilinearScorer(args.n_mlp_sec,args.n_mlp_sec,args.n_mlp_sec,init_std=args.init_std, rank = args.n_mlp_sec, factorize = args.factorize)
                                self.trilinear_sib_r = TrilinearScorer(args.n_mlp_sec,args.n_mlp_sec,args.n_mlp_sec,init_std=args.init_std, rank = args.n_mlp_sec, factorize = args.factorize)
                        if args.use_cop:
        ...
        def BNetEnergy(self, tree, bmask=None, grad=True, nsample=6):
            ...
            if self.use_sib:
                ...
                layer_sib_l = self.trilinear_sib_l(edge_node_sib_m, edge_node_sib_m, edge_node_sib_h)
                layer_sib_r = self.trilinear_sib_r(edge_node_sib_m, edge_node_sib_m, edge_node_sib_h)
                ...
                if self.blayer_sib is None: self.blayer_sib = self.layer_sib.repeat(nsample,1,1,1)
                me_sib = self.blayer_sib[bmask]
                if not grad: me_sib = me_sib.detach()
                E_sib = ((q_value.unsqueeze(-1))*(q_value.unsqueeze(-2))*me_sib).sum(dim=(-1,-2,-3))
            if self.use_gp:
                if self.blayer_gp is None: self.blayer_gp = self.layer_gp.repeat(nsample,1,1,1)
                me_gp = self.blayer_gp[bmask]
                if not grad: me_gp = me_gp.detach()
                E_gp = ((q_value.unsqueeze(-1))*(q_value.unsqueeze(-3))*me_gp).sum(dim=(-1,-2,-3))
            E = (q_value*unary_potential).sum(dim=(-1,-2)) + E_sib + E_gp + layer_sib.sum(dim=(-1,-2))
            return E
        ...
        def NetEnergy(self, tree, bmask=None, grad=True, verse=False):
                ...
                if not verse: 
                    E = (q_value*unary_potential).sum(dim=(-1,-2)) + E_sib + E_gp + layer_sib.sum(dim=(-1,-2)) #+ layer_gp.sum(dim=(-1,-2,-3)) #+ E_sib_lin.sum(-1) + E_gp_lin.sum(-1)
                    return E
                else: return (q_value*unary_potential).sum(dim=(-1,-2)), E_sib+layer_sib.sum(dim=(-1,-2)), E_gp#+layer_gp.sum(dim=(-1,-2,-3)) 
        ...
        def SEvolution(self, ptree, vmask=None, nsample=3):
            ...
            for i in range(1000):
                ...
                #    print('evolution energy: ' + str(E.sum().item()))
        ...
        def mutation(self, ptree, vmask, p=0.1):
        ...
        def run(self, patience=3, nsample=6):
            ...
            ptrees[batch_size:] = pargmax(ttrees, self.mask_unary.repeat(nsample-1,1,1)).float()
