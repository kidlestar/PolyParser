...
class TrilinearScorer(nn.Module):
    ...
    def forward(self, layer1, layer2, layer3):
        ...
        if not self.factorize:
            ...
            if layer1.dim()==3: layer = torch.einsum('nia,abc,njb,nkc->nijk', layer1, self.W, layer2, layer3)
            else: layer = torch.einsum('snia,abc,snjb,snkc->snijk', layer1, self.W, layer2, layer3)
